...
- ✅ Five size presets: `tiny` (~10M), `0.1b` (~100M), `0.5b` (~500M), `1b` (~1B), `custom`
...
- ✅ Bilingual docs (English code comments + Chinese explanations)

> 两种架构均基于 HF Transformers 从头实现；五种尺寸预设；WikiText-103 + GPT-2
> BPE tokenizer；基于 `Trainer` 的训练循环，支持 cosine 学习率、warmup、混合精度、
> 梯度检查点；一键 Colab 笔记本；中英双语文档。
...
├── configs/                        # yaml presets (for reference) / YAML 预设（参考用）
│   ├── tiny.yaml
│   ├── 0.1b.yaml
│   ├── 0.5b.yaml
│   ├── 1b.yaml
│   └── custom.yaml
├── llm_scratch/                    # the python package / Python 包
│   ├── __init__.py
│   ├── config.py                   # size presets + config builder / 尺寸预设与配置构建
│   ├── model_gpt2.py               # GPT-2 style model / GPT-2 风格模型
│   ├── model_llama.py              # LLaMA style model / LLaMA 风格模型
│   ├── dataset.py                  # WikiText-103 + tokenizer / 数据集与 tokenizer
│   ├── train.py                    # training entry / 训练入口
│   └── generate.py                 # generation script / 生成脚本
└── notebooks/
    └── train_llm_colab.ipynb       # one-click Colab notebook / 一键 Colab 笔记本
```
...
## 🚀 Quick start on Colab / Colab 快速开始

1. Open [Google Colab](https://colab.research.google.com/).
2. **Runtime → Change runtime type → T4 GPU** (or A100 on Pro).
3. Upload this whole folder (`llm-from-scratch/`) to Colab — easiest way:
   zip it locally, drag the zip into the Colab file browser, then `!unzip`.
...
> 1) 打开 Colab；2) 运行时 → 更改运行时类型 → T4 GPU；3) 上传本目录到 Colab
> （建议先打包成 zip 再上传、解压）；4) 在 Colab 中打开 `notebooks/train_llm_colab.ipynb`；
> 5) 从上到下依次运行单元格。**在第 4 节设置 `ARCH` 与 `SIZE`** —— 这就是"选择参数"。

### Smoke test (free T4, ~10 minutes) / 冒烟测试（免费 T4，约 10 分钟）
...
```bash
git clone <your-repo-url> llm-from-scratch
cd llm-from-scratch
...
### Use as a Python library / 作为 Python 库使用
...
On **WikiText-103**, training for **1 epoch** (≈ 1,150 chunks of 1024 tokens
for the full dataset, more for smaller `block_size`):
...
> 首次运行会下载 WikiText-103（约 180MB）；Colab 免费 T4 显存 16GB，`0.5b` 必
> 须开启梯度检查点 + 小 batch + 梯度累积；`1b` 在 Colab 上仅作演示，建议 A100 或
> 云 GPU；本项目为教学脚手架，若要真正训练可用 LLM，需更多数据、更长上下文、更多
> 训练步数与更大词表。
